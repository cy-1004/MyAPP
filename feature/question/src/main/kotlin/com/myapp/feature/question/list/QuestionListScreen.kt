...
import com.myapp.feature.question.ui.QuestionListItem

/**
 * 疑问列表（PRD 3.5）。
 *
 * 交互约定：
 *   - 列表按状态分三组：待解决 / 已解决 / 已归档，待解决优先
 *   - 左滑 = 删除，Snackbar 可撤销（软删除，恢复无损）
 *   - 点击 = 进编辑页
 *   - 搜索栏输入 = 实时 LIKE 搜索 content
 *   - 标签 chip = 切换标签筛选
 */
...
fun QuestionListScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text("疑问", style = MaterialTheme.typography.titleLarge) },
                navigationIcon = {
        ...
        floatingActionButton = {
            FloatingActionButton(
                ...
                Icon(Icons.Default.Add, contentDescription = "新建疑问")
            }
        },
    ) { innerPadding ->
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun QuestionGroups(
    state: com.myapp.feature.question.list.QuestionListUiState,
    onNavigate: (Route) -> Unit,
    ...
    LazyColumn(
        ...
        if (state.open.isNotEmpty()) {
            item(key = "header_open") { SectionHeader("待解决", state.open.size) }
            items(items = state.open, key = { "open-${it.id}" }) { q ->
                SwipeableQuestionRow(
                    question = q,
                    onDelete = { onDelete(q) },
                    onClick = { onNavigate(Route.QuestionDetail(q.id)) },
                    modifier = Modifier.animateItem(),
                )
            }
        }
        if (state.resolved.isNotEmpty()) {
            item(key = "header_resolved") { SectionHeader("已解决", state.resolved.size) }
            items(items = state.resolved, key = { "resolved-${it.id}" }) { q ->
                SwipeableQuestionRow(
                    question = q,
                    onDelete = { onDelete(q) },
                    onClick = { onNavigate(Route.QuestionDetail(q.id)) },
        ...
        if (state.archived.isNotEmpty()) {
            item(key = "header_archived") { SectionHeader("已归档", state.archived.size) }
            items(items = state.archived, key = { "archived-${it.id}" }) { q ->
                SwipeableQuestionRow(
                    question = q,
                    onDelete = { onDelete(q) },
                    onClick = { onNavigate(Route.QuestionDetail(q.id)) },
...
private fun SearchBar(
    ...
    OutlinedTextField(
        ...
        placeholder = { Text("搜索疑问内容", style = MaterialTheme.typography.bodyMedium) },
        leadingIcon = {
            Icon(
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun SwipeableQuestionRow(
    question: Question,
    onDelete: () -> Unit,
    onClick: () -> Unit,
    modifier: Modifier = Modifier,
    ...
    // 用 rememberUpdatedState 让它始终指向最新的那一份（与 NoteListScreen 同一坑）
    ...
    SwipeToDismissBox(
        ...
        QuestionListItem(question = question, onClick = onClick)
    }
}
...
private fun emptyTextFor(query: String, tag: String?): String = when {
    query.isNotBlank() -> "没找到包含「$query」的疑问"
    tag != null -> "带「$tag」标签的疑问还没有"
    else -> "一条疑问都还没记"
}
