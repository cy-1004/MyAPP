...
/**
 * 资讯列表用的投影行——**刻意不含 `content`**（PRD 3.9 / 4.5）。
 *
 ...
 * 再交给 Compose——这就是当年「每次进资讯页都要加载好久」的原因。
 * 详情页仍然查整行（那里确实要正文），走 `observeById`。
 *
 * **为什么带 [sourceTitle]**：它直接取自 query 里 JOIN 出来的 `rss_source.title`，
 * 不再回头单独查一次订阅源表。改版前的做法是「一批行拿到手后查一次 `sourceDao.getAll()`
 * 建映射」，一批只查一次很划算；但 `PagingData.map` 是**逐条**执行的，
 * 那套做法在分页下会退化成每篇文章查一次全表。query 本来就 JOIN 了 `rss_source`，
 * 把标题顺手带出来最省事。
 */
